...
DWORD WINAPI ThreadOperador(LPVOID param) {
	...
	COORD pos, comands;
	pos.X = 0; comands.X = 26;
	pos.Y = 12; comands.Y = 1;
	DWORD size;
	...
	while (!dados->terminar) {
		...
		if (_tcscmp(COM, TEXT("stop")) == 0) {
			...
			_tprintf(TEXT("EU: stop %d segundos\n"), cel.comando[1]);
			comands.Y++;
			...
			SetConsoleCursorPosition(hConsole, comands);
			_tprintf(TEXT("EU: invert faixa %d\n"), cel.comando[1]);
			comands.Y++;
			SetConsoleCursorPosition(hConsole, geral);
			...
			SetConsoleCursorPosition(hConsole, comands);
			_tprintf(TEXT("EU: insert faixa %d, coluna %d\n"), cel.comando[1], cel.comando[2]);
			comands.Y++;
			SetConsoleCursorPosition(hConsole, geral);
		}
